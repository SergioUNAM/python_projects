# Inventory operations — four reproducible demos

These demos turn four real telecommunications inventory processes into small, auditable examples using **fictional data only**. They preserve the operational rules and failure modes while removing company names, customer data, credentials, internal database details, and production configuration.
...
4. **SIM recommendation tracking** — compares cumulative delivery snapshots as a multiset, identifies ambiguous SIM/date assignments, builds a canonical row per SIM, and measures activation at 7/14/30/61 days plus 61-day retention for recommended platform/equipment pairs.
...
python -m unittest discover -s recommendation_tracking/tests -v
```
